...
namespace SoulFoodAiBack.Controllers
{
    ...
    public class RecipeController : ControllerBase
    {
        ...
        {{
            ...
            recipeAdd.Protein = ingredientesReceta.Sum(ri => (ri.Quantity * ri.Ingredient.Protein) / 100);
            recipeAdd.Carbs = ingredientesReceta.Sum(ri => (ri.Quantity * ri.Ingredient.Carbs) / 100);
            recipeAdd.Fat = ingredientesReceta.Sum(ri => (ri.Quantity * ri.Ingredient.Fat) / 100);
            recipeAdd.TotalKcal = (int)ingredientesReceta.Sum(ri => (ri.Quantity * ri.Ingredient.Kcal) / 100);
